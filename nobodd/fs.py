...
class FatDirectory(abc.Iterable):
    ...
    def _clean_entries(self):
        ...
        for read_offset, entry in self._iter_entries():
            if isinstance(entry, DirectoryEntry):
                if entry.filename[0] == 0xE5: # deleted entry
                    continue
            if isinstance(entry, LongFilenameEntry):
                if entry.sequence == 0xE5: # deleted entry
                    continue
    ...
    def _find_entry(self, entry):
        ...
        :meth:`_group_entries`) which matches the starting cluster of *entry*.
        ...
        *index*, returning them as a sequence.
        ...
        if lfn is None:
            ...
            entries = [
                ...
                in on_first(reversed(enumerate(range(len(lfn), 26), start=1)))
            ]
    ...
    def _get_lfn_sfn_ext(self, filename, dos_encoding='iso-8859-1'):
        ...
        *dos_encoding*), an extension (also encoded in *dos_encoding*), and
        ...
        sfn = sfn_safe(filename.lstrip('.').encode('dos_encoding', 'replace'))
        if b'.' in sfn:
        ...
        if len(sfn) <= 8 and len(ext) <= 3:
            try:
                attrs = {
                    sfn.upper() + b'.' + ext.upper(): 0,
                    sfn.upper() + b'.' + ext.lower(): 0b10000,
                    sfn.lower() + b'.' + ext.upper(): 0b01000,
        ...
        if sfn_only:
            ...
            if len(lfn) % 26:
                ...
                lfn = lfn.ljust(pad, b'\xff\xff')
            assert len(lfn) % 26 == 0
            ext = ext[:3]
            sfn = self._get_unique_sfn(sfn, ext)
        sfn = sfn.ljust(8, b' ').upper()
        ext = ext.ljust(3, b' ').upper()
    ...
    def create(self, filename):
        ...
        for cleaned in (False, True):
            ...
            try:
                for offset, entry in reversed(zip(offsets, entries)):
